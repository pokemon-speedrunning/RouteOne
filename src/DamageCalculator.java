...
public class DamageCalculator {
	...
	private static int damage(Move attack, Pokemon attacker, Pokemon defender,
		...
		if (Type.isPhysicalType(attack.getType())) {
			...
			a += 2;
			a = STAB ? a * 3 / 2 : a;
			a = Type.applyTypeEffectiveness(a, attack.getType(), defender
	...
	public static String summary(Pokemon p1, Pokemon p2, BattleOptions options) {
		...
		if (options.getVerbose() == BattleOptions.EVERYTHING) {
			...
			for (Move move : p1.getMoveset()) {
				int minDmg = Math.min(p2.getHP(),
						minDamage(move, p1, p2, mod1, mod2));
				if (minDmg > 0) {
					int minCritDmg = Math.min(p2.getHP(),
							minCritDamage(move, p1, p2, mod1, mod2));
					TreeMap<Integer, Double> dmgMap = detailedDamage(move, p1,
							p2, mod1, mod2, false);
					TreeMap<Integer, Double> critMap = detailedDamage(move, p1,
							p2, mod1, mod2, true);
					...
					for (Integer i : dmgMap.keySet()) {
						if ((i - minDmg) % 7 == 0) {
							sb.append(endl);
							if (i.intValue() == p2.getHP()
									&& minDmg != p2.getHP()) {
								sb.append(endl);
							}
							sb.append("            ");
						} else if (i.intValue() == p2.getHP()
								&& minDmg != p2.getHP()) {
							sb.append(endl);
							sb.append(endl);
							sb.append("            ");
						...
						sb.append(String.format("%3d: %6.02f%%     ", i,
								dmgMap.get(i)));
					}
					sb.append(endl);
					sb.append(endl);
		...
		if (options.getVerbose() == BattleOptions.EVERYTHING) {
			...
			for (Move move : p2.getMoveset()) {
				int minDmg = Math.min(p1.getHP(),
						minDamage(move, p2, p1, mod2, mod1));
				if (minDmg > 0) {
					int minCritDmg = Math.min(p1.getHP(),
							minCritDamage(move, p2, p1, mod2, mod1));
					TreeMap<Integer, Double> dmgMap = detailedDamage(move, p2,
							p1, mod2, mod1, false);
					TreeMap<Integer, Double> critMap = detailedDamage(move, p2,
							p1, mod2, mod1, true);
					// sb.append(String.format("%s (%s) ", p1.pokeName(),
					// p1.statsStr()));
					sb.append(move.getName());
					...
					for (Integer i : dmgMap.keySet()) {
						if ((i - minDmg) % 7 == 0) {
							sb.append(endl);
							if (i.intValue() == p1.getHP()
									&& minDmg != p1.getHP()) {
								sb.append(endl);
							}
							sb.append("            ");
						} else if (i.intValue() == p1.getHP()
								&& minDmg != p1.getHP()) {
							sb.append(endl);
							sb.append(endl);
							sb.append("            ");
	...
	private static TreeMap<Integer, Double> detailedDamage(Move attack,
			Pokemon attacker, Pokemon defender, StatModifier atkMod,
			StatModifier defMod, boolean crit) {
		TreeMap<Integer, Double> dmgMap = new TreeMap<Integer, Double>();
		for (int i = MIN_RANGE; i <= MAX_RANGE; i++) {
			int dmg = Math
					.min(defender.getHP(),
							damage(attack, attacker, defender, atkMod, defMod,
									i, crit));
	...
	private static String summary_help(Pokemon p1, Pokemon p2,
		...
		for (Move m : p1.getMoveset()) {
			...
			if (maxDmg != 0) {
				sb.append(String.format("%d-%d %.02f-%.02f%%", minDmg, maxDmg,
						minPct, maxPct));
			} else {
			...
			sb.append(String.format("%d-%d %.02f-%.02f", critMinDmg,
					critMaxDmg, critMinPct, critMaxPct));
			sb.append("%)" + endl);
			...
			if (maxDmg >= oppHP && minDmg < oppHP) {
				double oneShotPct = oneShotPercentage(m, p1, p2, mod1, mod2,
						false);
				sb.append(String.format("\t(One shot prob.: %.02f%%)",
						oneShotPct) + endl);
			}
			...
			while (minDmgWork < oppHP && hits < 5) {
				...
				if (maxDmgWork >= oppHP && minDmgWork < oppHP) {
					...
					double nShotPct = nShotPercentage(m, p1, p2, mod1, mod2,
							hits, 0);
					sb.append(String.format("\t(%d shot prob.: %.04f%%)", hits,
							nShotPct) + endl);
				}
			}
			...
			while (minDmgWork < oppHP && hits < 5) {
				...
				if (maxDmgWork >= oppHP && minDmgWork < oppHP) {
					...
					double nShotPct = nShotPercentage(m, p1, p2, mod1, mod2, 0,
							hits);
					sb.append(String
							.format("\t(%d crits death prob.: %.04f%%)", hits,
									nShotPct)
							+ endl);
				}
			}
			...
			for (int non = 1; non <= 5 && realminDmg * (non + 1) < oppHP; non++) {
				for (int crit = 1; non + crit <= 5
						&& realminDmg * (non + crit) < oppHP; crit++) {
					int sumMin = critMinDmg * crit + minDmg * non;
					int sumMax = critMaxDmg * crit + maxDmg * non;
					if (sumMin < oppHP && sumMax >= oppHP) {
						System.out.printf(
								"working out %d non-crits + %d crits\n", non,
								crit);
						double nShotPct = nShotPercentage(m, p1, p2, mod1,
								mod2, non, crit);
						sb.append(String
								.format("\t(%d non-crit%s + %d crit%s death prob.: %.04f%%)",
										non, non > 1 ? "s" : "", crit,
										crit > 1 ? "s" : "", nShotPct)
								+ endl);
					}
	...
	private static double nShotPercentage(Move attack, Pokemon attacker,
			Pokemon defender, StatModifier atkMod, StatModifier defMod,
			int numHitsNonCrit, int numHitsCrit) {
		int rawHitDamageNC = damage(attack, attacker, defender, atkMod, defMod,
				MAX_RANGE, false);
		...
		int rawHitDamageCR = damage(attack, attacker, defender, atkMod, defMod,
				MAX_RANGE, true);
		...
		if (numHitsNonCrit > 0) {
			for (int i = minDamageNC; i <= rawHitDamageNC; i++) {
				chances += nShotPctInner(minDamageNC, rawHitDamageNC,
						minDamageCR, rawHitDamageCR, rawHP, 0, i,
		...
		return 100.0
				* chances
				/ Math.pow(MAX_RANGE - MIN_RANGE + 1, numHitsNonCrit
						+ numHitsCrit);
	}

	private static double nShotPctInner(int minDamageNC, int maxDamageNC,
			int minDamageCR, int maxDamageCR, int hp, int stackedDmg,
			int rolledDamage, int hitsLeftNonCrit, int hitsLeftCrit,
			int[] probsNC, int[] probsCR) {
		...
		if (stackedDmg >= hp
				|| (stackedDmg + hitsLeftNonCrit * minDamageNC + hitsLeftCrit
						* minDamageCR) >= hp) {
			return Math.pow(MAX_RANGE - MIN_RANGE + 1, hitsLeftNonCrit
					+ hitsLeftCrit)
					* (wasCritical ? probsCR[rolledDamage - minDamageCR]
							: probsNC[rolledDamage - minDamageNC]);
		} else if (hitsLeftNonCrit == 0 && hitsLeftCrit == 0) {
			return 0;
		} else if (stackedDmg + hitsLeftNonCrit * maxDamageNC + hitsLeftCrit
				* maxDamageCR < hp) {
			return 0;
		} else {
			double chances = 0;
			if (hitsLeftNonCrit > 0) {
				for (int i = minDamageNC; i <= maxDamageNC; i++) {
					chances += nShotPctInner(minDamageNC, maxDamageNC,
							minDamageCR, maxDamageCR, hp, stackedDmg, i,
